/*
Esercizio: far girare la camera rispetto al centro, sempre coi cubi
*/
...
int main() {
    ...
    while (!glfwWindowShouldClose(window)) {
        ...
        //Camera che gira in cerchio attorno all'origine
        glm::mat4 projection = glm::mat4(1.0f); //projection transform
        projection = glm::perspective(glm::radians(45.0f), (float)SCR_WIDTH / (float)SCR_HEIGHT, 0.1f, 100.0f); //projection transform

        glm::mat4 view = glm::mat4(1.0f); //view transform
        //Definisco la traiettoria della telecamera (circonferenza di raggio 10)
        float radius = 10.0f; //raggio
        //Posizione -> faccio una traiettoria circolare attorno all'origine
        float camX = sin(glfwGetTime()) * radius;
        float camZ = cos(glfwGetTime()) * radius;

                           //posizione della camera     //mentre ruota guarda l'origine         //asse y su cui sto ruotando
        view = glm::lookAt(glm::vec3(camX, 0.0f, camZ), glm::vec3(0.0f, 0.0f, 0.0f), glm::vec3(0.0f, 1.0f, 0.0f)); //posiziono la camera avanti di -3.0
        ...
        for (unsigned int i = 0; i < 10; i++) {
            ...
            //Ogni cubo ruota in base al tempo -> se aggiungo i aumento man mano la velocità
            //model = glm::rotate(model, (float)glfwGetTime()+i, glm::vec3(0.5f, 1.0f, 0.0f));

            //Ogni 3 assegno una rotazione differente
            //float angle = 20.0f;
            //if (i % 3 == 0)
            //    angle = (float)glfwGetTime() * 25.0f; //Angolo dipendente dal tempo e aggiungo una rotazione di 25.0
